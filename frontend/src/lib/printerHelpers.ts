...
export const renderReceiptToCanvas = async (data: ReceiptData): Promise<HTMLCanvasElement> => {
    ...
    data.items.forEach((item) => {
        ...
        const nextY = wrapText(nameText, 15 + qtyWidth, y, nameMaxWidth, 20);
        ...
        if (chineseName && chineseName !== nameText) {
            ctx.font = '12px sans-serif'; // Slightly smaller for Chinese
            y = wrapText(`   ${chineseName}`, 15, y, width - 30, 18);
        }
        ...
        if (varList.length > 0) {
            ...
            y = wrapText(varText, 15, y, width - 30, 18);
        ...
        if (itemNote) {
            ...
            const noteYBefore = y;
            y = wrapText(noteText, 15, y, width - 30, 18);
