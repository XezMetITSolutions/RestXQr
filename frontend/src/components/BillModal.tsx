...
export default function BillModal({ isOpen, onClose, onPaymentComplete, order, restaurant, allowPartialPayment = false }: BillModalProps) {
  ...
  const handlePartialPayment = () => {
    ...
    const paymentNotification = {
      ...
      orderId: order.id,
      amount: paymentAmount,
      payerName: payerName || t('Anonim'),
      remainingAmount: remainingAmount - paymentAmount,
      timestamp: new Date().toISOString(),
      message: `${payerName || t('Bir müşteri')} ${paymentAmount.toFixed(2)} ₺ ${t('ödeme yaptı. Kalan:')} ${(remainingAmount - paymentAmount).toFixed(2)} ₺`
    };
  ...
  const handleMarkAsPaid = () => {
    ...
    if (paymentType === 'system') {
      ...
      const paymentNotification = {
        ...
        timestamp: new Date().toISOString(),
        message: t('Ödeme yapmak için cihazınızı kullanın')
      };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-2xl font-bold text-gray-800">{t('Fatura')}</h2>
          <button
            onClick={onClose}
        ...
        <div id="bill-content" className="p-6">
          ...
          <div className="text-center mb-8">
            ...
            {restaurant.phone && (
              <p className="text-gray-600 mb-1">{t('Tel')}: {restaurant.phone}</p>
            )}
            {restaurant.taxNumber && (
              <p className="text-gray-600">{t('Vergi No')}: {restaurant.taxNumber}</p>
            )}
          </div>
          ...
          <div className="mb-6">
            <div className="flex justify-between items-center mb-4">
              <div>
                <p className="text-gray-600">{t('Fatura No')}: #{order.id.toString().padStart(6, '0')}</p>
                <p className="text-gray-600">{t('Tarih')}: {new Date().toLocaleDateString('tr-TR')}</p>
                <p className="text-gray-600">{t('Saat')}: {new Date().toLocaleTimeString('tr-TR')}</p>
              </div>
              <div className="text-right">
                <p className="text-gray-600">{t('Masa')}: {order.tableNumber}</p>
                <p className="text-gray-600">{t('Misafir')}: {order.guests} {t('kişi')}</p>
              </div>
            </div>
          </div>
          ...
          <table className="w-full border-collapse border border-gray-300 mb-6">
            <thead>
              <tr className="bg-gray-50">
                <th className="border border-gray-300 px-4 py-2 text-left">{t('Ürün')}</th>
                <th className="border border-gray-300 px-4 py-2 text-center">{t('Adet')}</th>
                <th className="border border-gray-300 px-4 py-2 text-right">{t('Birim Fiyat')}</th>
                <th className="border border-gray-300 px-4 py-2 text-right">{t('Toplam')}</th>
              </tr>
            </thead>
            <tbody>
          ...
          <div className="text-right space-y-2">
            <div className="flex justify-between">
              <div className="flex justify-between">
                <span className="text-gray-600">{t('Ara Toplam')}:</span>
                <span className="font-medium">{subtotal.toFixed(2)} ₺</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">{t('KDV (%10)')}:</span>
              ...
              {totalPaid > 0 && (
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex justify-between text-sm">
                    <span className="text-blue-700">{t('Ödenen')}:</span>
                    <span className="font-medium text-blue-800">{totalPaid.toFixed(2)} ₺</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-blue-700">{t('Kalan')}:</span>
            ...
            {isPaid && (
              <div className="mt-6 p-4 bg-green-100 border border-green-300 rounded-lg text-center">
                ...
                <p className="text-green-800 font-medium">{t('Ödeme Alındı!')}</p>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-between items-center p-6 border-t bg-gray-50">
            ...
            {!isPaid && !showPartialPayment && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold mb-4 text-gray-800">{t('Ödeme Türü')}</h3>
                <div className="space-y-3">
                  <label className="flex items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-300 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-gray-800">{t('Sistem Ödemesi')}</div>
                      <div className="text-sm text-gray-600">{t('Müşteri kendi cihazından ödeme yapacak')}</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-300 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-gray-800">{t('Manuel Ödeme')}</div>
                      <div className="text-sm text-gray-600">{t('Garson/Kasa ile ödeme - Herkes kendi hesabını söyler')}</div>
                    </div>
                  </label>
                </div>
              </div>
          ...
          {showPaymentHistory && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-60">
              <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
                <div className="flex justify-between items-center p-6 border-b">
                  <h3 className="text-xl font-bold text-gray-800">{t('Ödeme Geçmişi')} - {t('Masa')} {order.tableNumber}</h3>
                  <button
                ...
                <div className="p-6">
                  {existingPayments.length === 0 ? (
                    <p className="text-gray-500 text-center py-8">{t('Henüz ödeme yapılmamış.')}</p>
                  ) : (
                        ...
                        <div key={payment.id} className="border border-gray-200 rounded-lg p-4">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              ...
                              <p className="text-sm text-gray-600">
                                {t('Ödeme Yöntemi:')} {payment.method === 'cash' ? t('Nakit') : payment.method === 'card' ? t('Kart') : t('Mobil')}
                              </p>
                            </div>
                            <span className={`px-2 py-1 rounded text-xs ${payment.isPartial ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'
                              }`}>
                              {payment.isPartial ? t('Kısmi') : t('Tam')}
                            </span>
                          </div>

                          {payment.items.length > 0 && (
                            <div className="mt-3">
                              <p className="text-sm font-medium text-gray-700 mb-2">{t('Ödenen Ürünler:')}</p>
          ...
          {showPartialPayment && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-60">
              <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
                <div className="flex justify-between items-center p-6 border-b">
                  <h3 className="text-xl font-bold text-gray-800">{t('Kısmi Ödeme')} - {t('Masa')} {order.tableNumber}</h3>
                  <button
                ...
                <div className="p-6">
                  ...
                  {/* Ödeme Türü Seçimi */}
                  <div className="mb-6">
                    <h4 className="text-lg font-semibold mb-4 text-gray-800">{t('Ödeme Türü')}</h4>
                    <div className="space-y-3">
                      <label className="flex items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-300 transition-colors">
                        <input
                          type="radio"
                      ...
                      <label className="flex items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-300 transition-colors">
                        ...
                        <div>
                          <div className="font-medium text-gray-800">{t('Manuel Ödeme')}</div>
                          <div className="text-sm text-gray-600">{t('Garson/Kasa ile ödeme')}</div>
                        </div>
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {t('Ödeme Yapan Kişi (Opsiyonel)')}
                    </label>
                    <input
                      type="text"
                      value={payerName}
                      onChange={(e) => setPayerName(e.target.value)}
                      placeholder={t('Adınızı girin...')}
                  ...
                  {/* Ürün Seçimi */}
                  <div className="mb-6">
                    <h4 className="text-lg font-semibold mb-4 text-gray-800">{t('Ödenecek Ürünleri Seçin')}</h4>
                    ...
                    {selectedItems.length > 0 && (
                      <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                        <div className="flex justify-between text-sm">
                          <span className="text-blue-700">{t('Seçili Ürünlerin Toplamı')}:</span>
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {t('Maksimum')}: {remainingAmount.toFixed(2)} ₺
                    </p>
                  </div>
                  ...
                  <div className="flex justify-end space-x-3">
                    ...
                    >
                      {t('Ödeme Yap')} ({partialAmount > 0 ? partialAmount.toFixed(2) : getSelectedItemsTotal().toFixed(2)} ₺)
                    </button>
                  </div>
                </div>
              </div>
