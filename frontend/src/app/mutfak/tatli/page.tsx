...
export default function TatliStation() {
  ...
  const STATION = 'tatli';
  const STATION_NAME = '🍰 Tatlı İstasyonu';
  const STATION_COLOR = '#EC4899';
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-rose-50">
      ...
      <div className="max-w-7xl mx-auto px-6 mb-6">
        <div className="flex gap-3 overflow-x-auto pb-2">
          <Link href="/mutfak/izgara" className="px-6 py-3 bg-white text-gray-700 rounded-xl font-semibold hover:bg-orange-50 border-2 border-orange-200">
            🔥 Izgara
          </Link>
          ...
          <Link href="/mutfak/tatli" className="px-6 py-3 rounded-xl font-bold text-white shadow-lg" style={{ backgroundColor: STATION_COLOR }}>
            🍰 Tatlı
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-2xl shadow-lg">
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-3">Tatlı İstasyonunda Sipariş Yok</h3>
                  ...
                  >
                    ...
                    <div className="space-y-3 mb-6">
                      {stationItems.map((item, idx) => (
                        <div key={idx} className="flex items-center gap-4 p-4 bg-pink-50 rounded-xl border-2 border-pink-200">
                          <div className="text-3xl font-black text-pink-600">{item.quantity}x</div>
                          <div className="flex-1">
                            <div className="font-bold text-lg text-gray-800">{item.name}</div>
                            {item.notes && <div className="text-sm text-pink-700 italic mt-1">📝 {item.notes}</div>}
                          </div>
                        </div>
                      ))}
                    </div>
